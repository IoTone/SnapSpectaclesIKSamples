        ...
        for (var i=0; i<distanceScripts.length; i ++) {
            if (distanceScripts[i].api) {
                if (script.overwriteVisual) {
                    distanceScripts[i].overwriteVisualizer(script.showVisualizer, script.defaultColor);
                }

                if (script.setTextColor) {
                    distanceScripts[i].setTextColor(script.passColor, script.defaultColor);
                }
...
function checkIfAllWithinRange() {
    var isInRange = true;    
    for (var i=0; i<distanceScripts.length; i ++) {
        if (!distanceScripts[i].isWithinRange()) {
            isInRange = false;
            break;
        }
    }
